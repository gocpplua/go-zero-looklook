...
创建数据库 looklook_usercenter && 导入deploy/looklook_usercenter.sql数据


拷贝sql到容器mysql的/mnt目录下
docker cp deploy/sql/looklook_order.sql mysql:/mnt 
docker cp deploy/sql/looklook_payment.sql mysql:/mnt 
docker cp deploy/sql/looklook_travel.sql mysql:/mnt 
docker cp deploy/sql/looklook_usercenter.sql mysql:/mnt 
...
use looklook_order
source /mnt/looklook_order.sql

use looklook_payment
source /mnt/looklook_payment.sql

use looklook_travel
source /mnt/looklook_travel.sql

use looklook_usercenter
source /mnt/looklook_usercenter.sql
...
5、 重建容器:
5.1. docker-compose -f docker-compose-env.yml stop mysql
5.2. docker-compose -f docker-compose-env.yml rm mysql
5.3. docker-compose -f docker-compose-env.yml create mysql
5.4. docker-compose -f docker-compose-env.yml start mysql
5.5. docker exec -it mysql mysql -uroot -p

6. 修改文件夹权限
sudo chown -R data chenqi1
```
